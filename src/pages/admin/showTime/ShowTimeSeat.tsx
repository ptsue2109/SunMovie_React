...
const ShowTimeSeat = (props: Props) => {
  ...
  const newSeatByST = seatsByST.filter(
    (item: any) => item?.showTimeId?._id === id
  );
  const dataSelected = stList.find((item: any) => item._id === id);
  const [form] = Form.useForm();
  const onFinish = (val: any) => {
    val.showTimeId = id;
    dispatch(createData(val))
      .unwrap()
      .then(() => {
        onReset();
        message.success("Tạo ghế thành công");
        setTimeout(() => {
          navigate(config.routes.AdminShowTimes);
        }, 2000);
      })
            ...
            <Space key={item?._id} className="flex flex-wrap">
              <Card>
                ...
                <Button size="middle" type="link">
                  <Link to={`/admin/seats/_v=${item?._id}`}>
                    {" "}
                    Xem chi tiết{" "}
                  </Link>
                </Button>
          ...
          <Custom>
            <Descriptions title=" Tổng quan">
              ...
              <Descriptions.Item label="Thời gian chiếu">
                <Text>
                  {formatTime(dataSelected?.startAt)} -{" "}
                  {formatTime(dataSelected?.endAt)}
                </Text>
              </Descriptions.Item>
