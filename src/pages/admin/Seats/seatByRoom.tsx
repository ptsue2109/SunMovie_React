...
const SeatByRoom = (props: Props) => {
    ...
    return (
        <div>
            ...
            >
                <Panel header="Thông tin rạp" key="1" className="site-collapse-custom-panel">
                    <p>Tên rạp: {roomSelect?.name} </p>
                    <p>Trạng thái rạp: {roomSelect?.status ? 'Đang hoạt động' : 'Dừng hoạt động'}</p>
                    <p>Số lượng ghế: {roomSelect?.rows * roomSelect?.columns}</p>
                </Panel>
                <Panel header="Thông tin ghế" key="2" className="site-collapse-custom-panel">
                    <RenderSeats
