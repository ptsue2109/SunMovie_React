...
const NewsSidebar = (props: Props) => {
   ...
   return (
      <div>
         <aside className="hidden lg:block lg:col-span-3 pl-6 border-l bg-slate-50 w-full">
            <section>
                     ...
                     categories?.map((item: any, index: any) => (
                        <li key={index}>
                           <Link to={`${config.routes.newsCate1}/${item.slug}`}>
                              <span className={`text-gray-500 block py-1 leading-7 transition duration-200 hover:text-black`}>
                                 {item.title}
                              </span>
                           </Link>
                        </li>
                     ...
                     top10?.map((item: any, index: any) => (
                        <li key={index}>
                           <NavLink to={`/post/${item.slug}`} className="sidebar-news-link">
                              <span
                                 className={`limit-line-2 block py-1 leading-7 text-gray-500 transition duration-200 hover:text-black`}
                              >
                                 {item.title}
                              </span>
                           </NavLink>
