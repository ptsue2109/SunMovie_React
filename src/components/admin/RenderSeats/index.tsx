...
const { Option } = Select;
const RenderSeats = ({ row, column, seats, setSeats, seatDetails, setSeatDetails, seatFile, setSeatFile, roomId }: Props) => {
    ...
    const findSelectSeat = () => {
        ...
        for (const key in seatDetails) {
            let colVal = seatDetails[key]?.filter((seatVal: any) => seatVal?.status == 3);
            arr = [...arr, colVal]
        }
                ...
                <div>
                    ...
                    <div>Loại ghế: {val?.seatTypeId?.name}
                        <Select
                            ...
                            onChange={(value: any) => { changeSeatType(val?._id, value) }}
                        >
    ...
    const changeSeatType = (id: any, val: any) => {
        ...
        dispatch(updateSeatThunk(payload)).unwrap()
            .then(() => {
                dispatch(getOneSBSTById(roomId));
    ...
    const onSelectChange = (newSelectedRowKeys: any) => {
        ...
        let selectArr: any[] = seatArr?.filter((item: any, index: any) => selectedRowKeys.includes(index + 1))
    ...
    const renderChoice = () => {
        ...
        return (
            <>
                ...
                <Modal title="Basic Modal" open={isModalOpen} onCancel={handleCancel} okButtonProps={{ style: { display: 'none' } }}>
                    <Form onFinish={onFinish} form={form}>
                        <div>
                            Trạng thái ghế:
                            <Select defaultValue={defaultStatus[0]?.name} onChange={(value: any) => getStatusChoice(value)}>
                                {defaultStatus?.map((item: any) => (
                                    <Option value={item?._id} key={item?.value}>
                                        {item?.name}
                                    </Option>
                        ...
                        <div>Loại ghế:
                            <Select defaultValue={seatType[0]?.name} onChange={(value: any) => getSeatTypeChoice(value)}>
                                {seatType?.map((item: any) => (
                                    <Option value={item?._id} key={item?._id}>
                                        {item?.name}
                                    </Option>
    ...
    return (
        <div className="flex overflow-hidden gap-3" >
            ...
            <div>
                {seatArrSelect && seatArrSelect?.map((item: any) => (
                    <div key={item?._id}>
                        {item?._id}
                    </div>
                ))}
